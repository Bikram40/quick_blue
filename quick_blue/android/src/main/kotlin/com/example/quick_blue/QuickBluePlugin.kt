...
class QuickBluePlugin : FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler {
  ...
  override fun onAttachedToEngine(
    ...
    bluetoothManager =
        flutterPluginBinding.applicationContext.getSystemService(Context.BLUETOOTH_SERVICE) as
            BluetoothManager
  }
    ...
    when (call.method) {
      ...
      "startScan" -> {
        ...
        if (!adapter.isEnabled()) {
          Log.v(TAG, "onScanFailed: Not Enable")
          //          return
        }
      ...
      "disconnect" -> {
        val deviceId = call.argument<String>("deviceId")!!
        val gatt =
            knownGatts.find { it.device.address == deviceId }
      ...
      "writeValue" -> {
//         Log.v(TAG, "write res: writedsafsafasd...")
        val deviceId = call.argument<String>("deviceId")!!
        val service = call.argument<String>("service")!!
        val characteristic = call.argument<String>("characteristic")!!
        ...
        val writeResult =
            gatt.getCharacteristic(service to characteristic)?.let {
              it.value = value
              gatt.writeCharacteristic(it)
            }
//        Log.v(TAG, "write res: writedfasd... $writeResult ")
        if (writeResult == true)
        //          Log.v(TAG, "write res: write...")
        result.success(null)
        else result.error("Characteristic unavailable", null, null)
      }
      else -> {
      ...
      object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
          ...
          if (newState == BluetoothGatt.STATE_CONNECTED && status == BluetoothGatt.GATT_SUCCESS) {
            sendMessage(
                messageConnector,
                mapOf("deviceId" to gatt.device.address, "ConnectionState" to "connected")
            )
          } else {
            cleanConnection(gatt)
            sendMessage(
                messageConnector,
                mapOf("deviceId" to gatt.device.address, "ConnectionState" to "disconnected")
            )
          }
        }
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
          ...
          gatt.services?.forEach { service ->
            ...
            service.characteristics.forEach { characteristic ->
              ...
              characteristic.descriptors.forEach { Log.v(TAG, "        Descriptor ${it.uuid}") }
            }
          }
        }
        ...
        override fun onMtuChanged(gatt: BluetoothGatt?, mtu: Int, status: Int) {
          if (status == BluetoothGatt.GATT_SUCCESS) {
            sendMessage(messageConnector, mapOf("mtuConfig" to mtu))
          }
        }
...
fun BluetoothGatt.getCharacteristic(serviceCharacteristic: Pair<String, String>) =
    getService(UUID.fromString(serviceCharacteristic.first))
